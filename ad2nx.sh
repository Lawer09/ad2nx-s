...
github_script_stdout() {
    ...
    if [[ -n "${GITHUB_TOKEN}" ]]; then
        ...
        curl -Ls "${GITHUB_RAW_BASE}/${SCRIPT_REPO}/${SCRIPT_BRANCH}/${file_path}"
    fi
}

github_script_download() {
    local file_path="$1"
    local output_path="$2"
    if [[ -n "${GITHUB_TOKEN}" ]]; then
        curl -Ls \
...
install() {
    bash <(github_script_stdout "install.sh")
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(github_script_stdout "install.sh") $version
    if [[ $? == 0 ]]; then
        echo -e "${green}更新完成，已自动重启 ad2nx，请使用 ad2nx log 查看运行日志${plain}"
        exit
    fi
...
config() {
    echo "ad2nx在修改配置后会自动尝试重启"
    vi /etc/ad2nx/config.json
    sleep 2
    restart
    check_status
    case $? in
        0)
            echo -e "ad2nx状态: ${green}已运行${plain}"
            ;;
        1)
            echo -e "检测到您未启动ad2nx或ad2nx自动重启失败，是否查看日志？[Y/n]" && echo
            read -e -rp "(默认: y):" yn
            [[ -z ${yn} ]] && yn="y"
            if [[ ${yn} == [Yy] ]]; then
               show_log
            ...
            ;;
        2)
            echo -e "ad2nx状态: ${red}未安装${plain}"
    esac
}

uninstall() {
    confirm "确定要卸载 ad2nx 吗?" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
    ...
    echo ""
    echo -e "卸载成功，如果你想删除此脚本，则退出脚本后运行 ${green}rm /usr/bin/ad2nx -f${plain} 进行删除"
    echo ""
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        echo -e "${green}ad2nx已运行，无需再次启动，如需重启请选择重启${plain}"
    else
        if [[ x"${release}" == x"alpine" ]]; then
            service ad2nx start
        else
        ...
        check_status
        if [[ $? == 0 ]]; then
            echo -e "${green}ad2nx 启动成功，请使用 ad2nx log 查看运行日志${plain}"
        else
            echo -e "${red}ad2nx可能启动失败，请稍后使用 ad2nx log 查看日志信息${plain}"
        fi
    fi
...
stop() {
    ...
    check_status
    if [[ $? == 1 ]]; then
        echo -e "${green}ad2nx 停止成功${plain}"
    else
        echo -e "${red}ad2nx停止失败，可能是因为停止时间超过了两秒，请稍后查看日志信息${plain}"
    fi
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        echo -e "${green}ad2nx 重启成功，请使用 ad2nx log 查看运行日志${plain}"
    else
        echo -e "${red}ad2nx可能启动失败，请稍后使用 ad2nx log 查看日志信息${plain}"
    fi
    if [[ $# == 0 ]]; then
...
status() {
    if [[ x"${release}" == x"alpine" ]]; then
        service ad2nx status
    else
        systemctl status ad2nx --no-pager -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}ad2nx 设置开机自启成功${plain}"
    else
        echo -e "${red}ad2nx 设置开机自启失败${plain}"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}ad2nx 取消开机自启成功${plain}"
    else
        echo -e "${red}ad2nx 取消开机自启失败${plain}"
    fi
...
show_log() {
    if [[ x"${release}" == x"alpine" ]]; then
        ...
        journalctl -u ad2nx.service -e --no-pager -f
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
check_enabled() {
    if [[ x"${release}" == x"alpine" ]]; then
        temp=$(rc-update show | grep ad2nx)
        if [[ x"${temp}" == x"" ]]; then
            return 1
        else
            return 0
        ...
        temp=$(systemctl is-enabled ad2nx)
...
check_uninstall() {
    ...
    if [[ $? != 2 ]]; then
        echo ""
        echo -e "${red}ad2nx已安装，请不要重复安装${plain}"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 1
...
show_status() {
    ...
    case $? in
        0)
            ...
            ;;
        1)
            echo -e "ad2nx状态: ${yellow}未运行${plain}"
            show_enable_status
            ;;
        2)
            echo -e "ad2nx状态: ${red}未安装${plain}"
...
add_node_config() {
    ...
    if [[ "$isreality" != "y" && "$isreality" != "Y" && ( "$istls" == "y" || "$istls" == "Y" ) ]]; then
        ...
        if [ "$certmode" != "http" ]; then
            echo -e "${red}请手动修改配置文件后重启ad2nx！${plain}"
        fi
    fi
...
generate_config_file() {
    echo -e "${yellow}ad2nx 配置文件生成向导${plain}"
    echo -e "${red}请阅读以下注意事项：${plain}"
    echo -e "${red}1. 目前该功能正处测试阶段${plain}"
    echo -e "${red}2. 生成的配置文件会保存到 /etc/ad2nx/config.json${plain}"
        ...
        cores_config+="
    {
        ...
        \"Log\": {
            \"Level\": \"error\",
            \"ErrorPath\": \"/etc/ad2nx/error.log\"
        },
        \"OutboundConfigPath\": \"/etc/ad2nx/custom_outbound.json\",
        \"RouteConfigPath\": \"/etc/ad2nx/route.json\"
    },"
    fi
        ...
        cores_config+="
    {
        ...
        \"OriginalPath\": \"/etc/ad2nx/sing_origin.json\"
    },"
    fi
...
EOF
    ...
    cat <<EOF > /etc/ad2nx/custom_outbound.json
    [
        {
            "tag": "IPv4_out",
            "protocol": "freedom",
...
masquerade:
  type: 404
EOF
    echo -e "${green}ad2nx 配置文件生成完成，正在重新启动 ad2nx 服务${plain}"
    restart 0
    before_show_menu
}
...
show_usage() {
    ...
    echo "ad2nx update x.x.x - 安装 ad2nx 指定版本"
...
————————————————
  ...
  ${green}2.${plain} 更新 ad2nx
  ${green}3.${plain} 卸载 ad2nx
————————————————
  ${green}4.${plain} 启动 ad2nx
  ${green}5.${plain} 停止 ad2nx
  ${green}6.${plain} 重启 ad2nx
  ${green}7.${plain} 查看 ad2nx 状态
  ${green}8.${plain} 查看 ad2nx 日志
————————————————
  ${green}9.${plain} 设置 ad2nx 开机自启
  ${green}10.${plain} 取消 ad2nx 开机自启
————————————————
  ${green}11.${plain} 一键安装 bbr (最新内核)
  ${green}12.${plain} 查看 ad2nx 版本
  ${green}13.${plain} 生成 X25519 密钥
  ${green}14.${plain} 升级 ad2nx 维护脚本
  ${green}15.${plain} 生成 ad2nx 配置文件
 ...
 #后续更新可加入上方字符串中
    ...
    case "${num}" in
        ...
        11) install_bbr ;;
        12) check_install && show_ad2nx_version ;;
...
if [[ $# > 0 ]]; then
    case $1 in
        ...
        "version") check_install 0 && show_ad2nx_version 0 ;;
