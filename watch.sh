...
init_node_targets_file() { init_targets_file "${NODE_TARGETS_FILE}" "node_exporter"; }
init_process_targets_file() { init_targets_file "${PROCESS_TARGETS_FILE}" "process_exporter"; }
...
normalize_node_targets_file() { normalize_targets_file "${NODE_TARGETS_FILE}" "node_exporter"; }
normalize_process_targets_file() { normalize_targets_file "${PROCESS_TARGETS_FILE}" "process_exporter"; }
...
manage_prometheus_process_targets_menu() {
  while true; do
    ...
    case "${sub_choice}" in
      ...
      4) normalize_process_targets_file; echo "已完成规范化和去重。"; list_process_targets ;;
      0) break ;;
      *) echo "无效选项，请重新输入。" ;;
    esac
  done
...
manage_prometheus_menu() {
  while true; do
    ...
    echo "4. node_exporter 管理（添加/删除/查看）"
    echo "5. process_exporter 管理（添加/删除/查看）"
...
main_menu() {
  while true; do
    ...
    read -rp "请选择: " choice
    case "${choice}" in
      1) manage_prometheus_menu ;;
      2) manage_node_exporter_menu ;;
      3) manage_process_exporter_menu ;;
      0) echo "退出。"; exit 0 ;;
      *) echo "无效选项，请重新输入。" ;;
    esac
  done
